...
const SingleListItem = ({ item }: { item: Product }) => {
  ...
  const legacyImages = {
    thumbnails: item.imgs?.thumbnails?.length
      ? item.imgs.thumbnails
      : item.images?.length
      ? item.images
      : [productImage],
    previews: item.imgs?.previews?.length
      ? item.imgs.previews
      : item.images?.length
      ? item.images
      : [productImage],
  ...
  const handleQuickViewUpdate = () => {
    dispatch(
      updateQuickView({ ...item, imgs: legacyImages, discountedPrice })
    );
  };
  ...
  const handleProductDetails = () => {
    dispatch(
      updateproductDetails({ ...item, imgs: legacyImages, discountedPrice })
    );
  };
  ...
  return (
    <div
      ...
      className={`group rounded-xl bg-white dark:bg-[#131c2e] border border-gray-3/60 dark:border-slate-800 shadow-1 dark:shadow-[0_8px_20px_rgba(0,0,0,0.35)] cursor-pointer transition-all duration-700 ease-out ${
        isVisible
          ? "opacity-100 translate-y-0 scale-100"
          : "opacity-0 translate-y-7 scale-[0.98] pointer-events-none"
      } hover:-translate-y-1 hover:shadow-lg dark:hover:border-slate-700 overflow-hidden`}
    >
      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4 bg-gray-1 dark:bg-slate-800/40">
          <Image
            src={productImage}
            ...
            height={250}
            loading="lazy"
            className="w-auto h-auto max-h-[220px] object-contain transition-transform duration-300 group-hover:scale-105"
          />
          ...
          <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0 z-10">
            <button
              ...
              className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-dark dark:text-slate-200 bg-white dark:bg-slate-800 hover:text-blue dark:hover:text-blue-light border border-transparent dark:border-slate-700"
            >
              <svg
          ...
          <button
            ...
            className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-blue text-white ease-out duration-200 hover:bg-blue-dark shadow-sm"
          >
            Add to cart
          </button>
          ...
          <button
            ...
            className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 bg-white dark:bg-slate-800 border border-transparent dark:border-slate-700 ${
              isInWishlist
      ...
      <div className="w-full flex flex-col gap-5 sm:flex-row sm:items-center justify-center sm:justify-between py-5 px-4 sm:px-7.5 lg:pl-11 lg:pr-12">
        <div>
          <h3
            className="font-medium text-dark dark:text-white ease-out duration-200 hover:text-blue dark:hover:text-blue-light mb-1.5 text-base sm:text-lg"
          >
            {item.title}
          </h3>
          ...
          <span className="flex items-center gap-2 font-medium text-lg">
            ...
            <span className="text-dark-4 dark:text-slate-500 line-through text-sm">${item.price}</span>
          </span>
        </div>
